...
from packag.modules.pipeline.operations.extractors.fileToPrestadorExtractor.penedoFileToPrestadorExtractor import PenedoFileToPrestadorExtractor
...
PRESTADORES = [
    (
        "590.pdf",
        {
            "cnpj": "15.392.382/0001-13",
            "cpf": None,
            "inscricao_municipal": "7266",
            "razao_social": "RICCON CONSULTORIA EMPRESARIAL LTDA ME",
            "endereco": "LOT SANTANA ,DOM CONSTANTINO. 390 LOT 38",
            "numero": None,
            "bairro": None,
            "municipio": "Penedo",
            "uf": "AL",
            "cep": "57200000",
            "telefone": None,
            "email": "riccon.consultoria@hotmail.com"
        }
    ),
    (
        "646.pdf",
...
def test_if_all_extractor_methods_are_returning_the_correct_value(xml_filename, expected):
    ...
    extractor = PenedoFileToPrestadorExtractor(dto_file)
...
def test_if_extract_data_raise_extract_method_error_when_extract_text_from_pdf_raises_filenotfounderror():
    ...
    extractor = PenedoFileToPrestadorExtractor(MagicMock(
        file_path=Path('static/notas_fiscais/penedo/000.pdf'),
        file_extension='pdf'
    ))
