...
from packag.modules.pipeline.operations.extractors.fileToTomadorExtractor.arapiracaFileToTomadorExtractor import (
    ArapiracaFileToTomadorExtractor
)
...
TOMADORES = [
    (
        "205.pdf",
        {
            "cnpj": "12.517.413/0002-08",
            "cpf": None,
            "inscricao_municipal": "30210",
            "razao_social": "SERVICO DE A. AS MEP EMP.DE AL-SEBRAE-AL",
            "endereco": "ENGENHEIRO GORDILHO DE CASTRO, 80",
            "numero": None,
            "bairro": "CENTRO",
            "municipio": "Arapiraca",
            "uf": "AL",
...
def test_if_all_extractor_methods_are_returning_the_correct_value(xml_filename, expected):
    ...
    extractor = ArapiracaFileToTomadorExtractor(dto_file)
...
def test_if_extract_data_raise_extract_method_error_when_extract_text_from_pdf_raises_filenotfounderror():
    ...
    extractor = ArapiracaFileToTomadorExtractor(MagicMock())
